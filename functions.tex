...
For example all algebraic signatures include function symbols for addition and multiplication, but also constants, which are understood as functions of arity $0$.
...
We will begin by defining both approaches and comparing those with a simple example.
...
This procedure encodes a non-relational structure as a relational one, on which Relational Colour Refinement can be performed.
...
Since the following will heavily depend on this construction, let us consider an example.
...
\begin{figure}
	...
	\begin{subfigure}{0.49\textwidth}
		...
		\end{tikzpicture}
		\caption{The structure $\mathfrak A$. Green states represent $R^{\mathfrak A}$.}
...
\begin{definition}[RCR for structures with unary functions]
	Let $\sigma$ be a signature and let $\mathfrak A$ and $\mathfrak B$ be $\sigma$-structures.
...
\begin{definition}[$\mathsf{nfGF}(\mathsf C)$]
	...
	Concretely, for a relation symbol of arity $k$ and a function symbol of arity $\ell$, we only allow atomic formulae of the form $R(x_1,\dots,x_\ell)$, $x=y$ and $f(x_1,\dots,x_\ell)=y$.
...
Consider the structures $\mathfrak A$ and $\mathfrak B$ from Figure \ref{nRCRvsRCRk} and the formula $\phi=\exists^{\geq 1} x.(f(f(f(x)))=x)$ which utilises term nesting to find a cycle of length three.
...
Concretely, for a function symbol $f$, a term $s(x)$ and a natural number $k$, allow only terms of the form $f^m(s(x))$ where $m\leq\vert\mathfrak A\vert$ and with only up to $k$ alternations of function applications.
...
When considering $x,f(x)$, $f^2(x)$, $f^3(x)$ and so forth, until $f^m(x)$, where $m>\vert\mathfrak A\vert$, there have to be two numbers $i$ and $j$, such that $f^i(x)=f^j(x)$.
Therefore, we can decompose the path formed by $f^1(x),\dots,f^m(x)$ into a path to a cycle, the cycle itself, and a last part of that cycle.
...
\begin{proof}
	...
	Assume that $t(x_1)$ is of the form $g^m(s(x_1))$ for a unary function symbol $g$, a $m\in\mathbb N$ and a term $s=f^{m'}(s'(x))$ where $f\neq g$.
	...
	where 
	\begin{align*}
		\Phi'_{(k,\ell,p)}\coloneqq \{g^{k+p}(t'(x_1))=x_2 &\land g^{k}(t'(x_1))=g^{k+\ell}(t'(x_1)) \\
		& \land \operatorname{E}^{k,\ell}_g(t'(x_1)) \land \bigwedge_{0<\ell'<\ell} g^{k}(t'(x_1))\neq g^{k+\ell'}(t'(x_1)) \\
	...
	Then there is a
	\begin{align*}
		\phi(x_1,x_2)\coloneqq g^{k+p}(t'(x_1))=x_2 &\land g^{k}(t'(x_1))=g^{k+\ell}(t'(x_1)) \\
		& \land \operatorname{E}^{k,\ell}_g(t'(x_1)) \land \bigwedge_{0<\ell'<\ell} g^{k}(t'(x_1))\neq g^{k+\ell'}(t'(x_1)) \\
	...
	\begin{align*}
		...
		\Leftrightarrow A,a_1,a_2,a_3 \models &g^{k+p}(x_3)=x_2 \land g^{k}(x_3)=g^{k+\ell}(x_3) \\
		& \land \operatorname{E}^{k,\ell}_g(x_3) \land \bigwedge_{0<\ell'<\ell} g^{k}(x_3)\neq g^{k+\ell'}(x_3) \\
...
\begin{proof}
	...
	$$\mathfrak A,a_1,\dots,a_m,b_1,\dots,b_m\models R(y_1,\dots,y_m)\land t_1(x_1)=y_1\land\dots\land t_m(x_m)=y_m$$
...
This lemma also allows the usage of term equations, that is of formulae $t(x_1)=s(x_2)$ for term $t$ and $s$, as the equation symbol can be understood as an infix, binary relation.
...
\begin{align*}
	...
	& g^{k'+p'}(f^{k+p}(x))=y \land g^{k'}(f^{k+p}(x))=g^{k'+\ell'}(f^{k+p}(x)) \\
	& \operatorname{E}^{k',\ell'}_f(f^{k+p}(x)) \land \bigwedge_{0<\hat{\ell}<\ell'} g^{k'}(f^{k+p}(x))\neq g^{k'+\hat{\ell}}(f^{k+p}(x)) \\
	& f^k(x)=f^{k+l}(x) \land \operatorname{E}^{k,\ell}_f(x) \land \bigwedge_{0<\hat{\ell}<\ell} f^k(x)\neq f^{k+\hat{\ell}}(x)
	)
\end{align*}
...
\begin{proof}
	...
	Using \cref{TranslationOfArbAtomics} we can obtain a formula $\theta_\psi$ for every atomic subformula $\psi$ of $\phi$ which is equivalent to $\psi$ for structures with $n$ elements.
	...
	With this, we can transform $\phi'$ to a formula $\widetilde{\phi}\in\GFC$ of signature $\widetilde{\sigma}$, such that $\mathfrak A\models \phi'$ if, and only if, $\widetilde{\mathfrak A}\models \widetilde{\phi}$.
...
\begin{definition}[Total structures]
	...
	Let $\sigma= \sigma_{\operatorname{Rel}} \operatorname{\dot{\cup}} \sigma_{\operatorname{Func}}$ be a signature where $\sigma_{\operatorname{Func}}$ contains exactly all function symbols and is not empty.
...
Total structures capture the notion that every relation that encodes a function is defined for the complete domain of the function.
However, functions only valuate to exactly one value.
...
\begin{proof}
	...
	\begin{proof}
		...
		It holds that $\phi(\mathbf c')=\phi'(\mathbf c)$, because for all $x \in \set(\mathbf c)\setminus \{v_{y,z}\}$, it follows that $x\in\set(\mathbf c')$ and $\phi(x)=\phi'(x)$.
...
Now that we have proved that it is generally possible to enforce a distinguishing, acyclic structure to be functional, we want to proceed by proving that totality cannot be enforced.
...
Therefore, there exists an acyclic structure that distinguishes the encodings by homomorphism counts.
It now only remains to show that this structure cannot be total, acyclic and distinguishing.
...
We found that naive RCR cannot be characterised by acyclic structures, as the totality of functions inherently result in cycles. 
