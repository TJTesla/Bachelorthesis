...
Consider the formula $\phi=\exists^{\geq 1} x.(f(f(f(x)))=x)$ which utilizes term nesting to find a cycle with length three.
...
The key idea of this section will be, to encode a function $f$ as a family of relations.
This way, we can capture the notion of nesting function applications and thus construct the transitive closure.
However, a bound is necessary to ensure that the expanded signature is still finite, thus we will fixate a maximal alternation depth when discussing our new variant of $\RCR$.
Additionally, for the sake of simplicity we will only consider unary function symbols for now.
...
\begin{definition}[Transitive Expansion]
	...
	where we define $\operatorname{id}^{\mathfrak A}(a)\coloneqq a$ and 
...
For the sake of simplicity we will define the transitive expansion with alternation depth $1$ and because $\Vert\mathfrak A\Vert$ we will use $\operatorname{Alters}^1_2(\sigma)$ to do so.
When looking at the definition, we see that $\operatorname{Alters}^1_2(\sigma)=\{\operatorname{id}, f,f^2,g,g^2\}$ and as such 
$$\widetilde{\sigma}=\{R_{\operatorname{id}}, {f}, R_{f^2}, R_g, R_{g^2}, \operatorname{Eq}_{\operatorname{id},\operatorname{id}}, \operatorname{Eq}_{\operatorname{id},f}, \operatorname{Eq}_{\operatorname{id}, f^2}, \dots, \operatorname{Eq}_{g^2, g^2}\}.$$
...
\begin{figure}[h]
	...
	\begin{multicols}{2}
		\begin{tikzpicture}[node distance=3cm]
			...
			\draw 
			(a) edge[->, above, bend left] node{$f$} (b)
			(b) edge[->, below, bend left] node{$f$} (a)
			(a) edge[->, loop left] node{$g$} (a)
			(b) edge[->, loop right] node{$g$} (b);
		\end{tikzpicture}
		\begin{tikzpicture}[node distance=3cm]
			\node[state] (a) {$a$};
			\node[state, right=of a] (b) {$b$};
			...
			\draw 
			(a) edge[->, above, bend left, rwth-blue] node{\phantom{$f$}} (b)
			(b) edge[->, below, bend left, rwth-blue] node{} (a)
			(a) edge[->, loop left, rwth-red] node{} (a)
			(b) edge[->, loop right, rwth-red] node{} (b);
		\end{tikzpicture}
	\end{multicols}
...
With the knowledge of how the transitive expansion works, we can now define $\RCR$ for signatures that include unary function symbols.
...
In the following, we want to investigate, how much stringer this new algorithm is, by finding a logic that characterises it.
...
With this, we can prove the first result, which allows us to use any $f^m(x)=y$ in a formula.

\begin{lemma}
	Let $\psi(x_1,x_2)$ be of the form $f^m(x_1)=x_2$. 
...
\begin{proof}
	...
	$f^{k+p}(x_1)=x_2$ is being satisfied.
	...
	$f^{k}(x_1)=f^{k+\ell}(x_1)$ is being satisfied.
	Consider that
	...
	$\operatorname{E}^{k,\ell}_f(x_1)$ is being satisfied.
	...
	Assume that $\mathfrak A,a_1,a_2\models \theta(x_1,x_2)$ and that there are $\zeta_{(k,\ell,p)}(x_1,x_2)$ and $\zeta_{(k',\ell',p')}(x_1,x_2)$ with $(k,\ell,p)\neq (k',\ell',p')$, $\mathfrak A,a_1\models \exists^{\geq 1}x_2.\zeta_{(k,\ell,p)}(x_1,x_2)$ and $\mathfrak A,a_1\models \exists^{\geq 1}x_2.\zeta_{(k',\ell',p')}(x_1,x_2)$.
...
\begin{lemma}
	Let $\psi(x_1,x_2)$ be of the form $t(x_1)=x_2$ for a term $t$. 
...
\begin{proof}
	...
	By induction hypothesis, we have a formula $\theta_{s}(x_1,x_2)$ of the form $\bigvee \Phi_s(x_1,x_2)$ in the above defined form with $\mathfrak A,a_1,a_2 \models s(x_1)=x_2$ if, and only if, $\mathfrak A,a_1,a_2\models \theta_{s}(x_1,x_2)$.
...
This bound can be seen in the definition of the transitive expansion, but it is easy to see that the above proofs do not alter the alternation depth.
...
\begin{proof}
	...
	We will construct this formula $\phi'$ inductively and directly proof the equivalence.
