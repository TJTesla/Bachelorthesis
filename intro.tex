...
\textbf{Theorem \ref{}}:
There exist two families of structures $(\mathfrak A_i)_{i\in\mathbb N}$ and $(\mathfrak B_i)_{i\in\mathbb N}$ such that naive $\RCR$ distinguishes $\mathfrak A_i$ and $\mathfrak B_i$ for all $i>4$, but there does not contain a total, functional and acyclic structure that distinguishes $\mathfrak A_i$ and $\mathfrak B_i$ by number of homomorphism. \todo{adapt to actual notation}
