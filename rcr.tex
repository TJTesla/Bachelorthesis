...
Because of that, the $6$-ary relation includes one tuple, containing all $6$ elements each.
...
A guard in $\GFC$ uses only one edge, since atomic formulae are defined as equalities and relations.
...
As can be seen in \cite{brault-baron2014HypergraphAcyclicity}, there are multiple such definitions for hypergraphs, which can be applied to relational structures as well.
